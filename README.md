...
- `make build`
  ...
  - frontend
    the react Frontend which needs to be provided with all three indgredients which are generated by `make generate`, as well as all npm dependencies which are installed by `make install`. Can also be build independently by `make playground-build`.
  - backend
...
- `make docker-run`
  Starts the Docker container of playground udner the [sysbox](https://github.com/nestybox/sysbox) runtime. Make sure that you have the sysbox docker runtime installed on your computer. Instructions on how to install it can be found [here](https://github.com/nestybox/sysbox#installation).
- `make ci`
