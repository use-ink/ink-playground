...
use base_db::{
    ...
    Edition, Env, FileId, LangCrateOrigin,
};
use cfg::CfgOptions;
    ...
    fn from(crate_graph_json: &CrateGraphJson) -> Self {
        ...
        for (_, data) in &crate_graph_json.crates {
            ...
            crate_graph.add_crate_root(
                ...
                edition,
                display_name,
                data.version.as_ref().map(|val| val.to_owned()),
                cfg_options,
...
impl From<&CrateData> for CrateDataJson {
    fn from(crate_data: &CrateData) -> Self {
        ...
        CrateDataJson {
            ...
            edition,
            display_name,
            version: crate_data.version.as_ref().map(|val| val.to_string()),
            cfg_options,
            potential_cfg_options,
            env,
            proc_macro,
    ...
    fn from(crate_origin: &CrateOrigin) -> Self {
        match crate_origin {
            CrateOrigin::Lang(lang) => {
                CrateOriginJson::Lang(LangCrateOriginJson::from(lang))
            }
